...
let simpleUpgrade = (() => {
    var ref = _asyncToGenerator(function* (version) {
        let spinner;
        ...
        powershell.runSimpleUpgrade(version).then((() => {
            var ref = _asyncToGenerator(function* (output) {
                ...
                const installedVersion = yield versions.getInstalledNPMVersion();
...
let upgrade = (() => {
    var ref = _asyncToGenerator(function* (version, npmPath) {
        let spinner;
        ...
        npmpathfinder(npmPath).then(function (confirmedPath) {
            powershell.runUpgrade(version, confirmedPath).then((() => {
                var ref = _asyncToGenerator(function* (output) {
...
let prepareUpgrade = (() => {
    var ref = _asyncToGenerator(function* (_program) {
        ...
        if (program.prompt && !(yield askForConfirmation())) return;

        // Check Execution Policy
        debug('Upgrade: Checking execution policy');
        const canExecute = yield powershell.checkExecutionPolicy();
        ...
        const isOnline = yield checkForInternet();
        ...
        if (!program.npmVersion) {
            ...
            const availableVersions = yield versions.getAvailableNPMVersions();
            const versionList = [{
                type: 'list',
                name: 'version',
